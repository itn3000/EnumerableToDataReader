...
namespace EnumerableToDataReader.Test
{
    ...
    public class TestTypedDataReader
    {
        ITestOutputHelper m_Outputter;
        public TestTypedDataReader(ITestOutputHelper outputter)
        {
        ...
        public void TestRead()
        {
            var data = Enumerable.Range(0, 100).Select(i =>
            {
                return new SampleClass()
                {
                    ...
                    j = i
                    ,
                    k = Enumerable.Range(0,32).Select(j => (byte)i).ToArray()
                };
            }).ToArray();
            var ti = typeof(SampleClass).GetTypeInfo();
            var properties = ti.GetProperties();
            using (var reader = data.AsDataReader())
            {
                int i = 0;
                while (reader.Read())
                {
                    ...
                    var charsread = reader.GetChars(reader.GetOrdinal("a"), 0, ch, 0, ch.Length);
                    Assert.Equal(data[i].a.Length, charsread);
                    i++;
                }
            }
        }
